...
# Nouvelle logique : on construit chaque colonne caractère par caractère, mais on n'insère un mot que s'il tient entièrement dans la place restante, sinon on le porte à la colonne suivante (jamais coupé)
RIGHT_COL=()
FAR_COL=()
WORDS_SHUFFLED=($(shuf -e "${WORDS[@]}"))
word_count=0
word_limit=11
min_words=5
carry_word=""
...
for ((i=0; i<11; i++)); do
    ...
    for col in 1 2; do
        line=""
        # Si on a un mot à reporter, on commence la colonne directement par ce mot
        if [[ -n "$carry_word" ]]; then
            if (( carry_wlen <= 12 )); then
                line+="$carry_word"
                ((word_count++))
                carry_word=""
                carry_wlen=0
            else
                # Mot trop long pour une colonne (devrait jamais arriver)
                carry_word=""
                carry_wlen=0
            fi
        fi
        while [[ ${#line} -lt 12 ]]; do
            insert_word=0
            # On force l'insertion d'un mot si on n'a pas atteint le minimum
            if (( word_count < min_words )); then
                insert_word=1
            elif (( word_count < word_limit )) && (( RANDOM % 11 == i )); then
                insert_word=1
            fi
            if (( insert_word == 1 )); then
                w=${WORDS_SHUFFLED[$word_count]}
                wlen=${#w}
                # Probability: 5-7 chars = high, 3-4 or 8-9 = medium, <3 or >9 = low
                if (( wlen >= 5 && wlen <= 7 )); then
                    chance=80
                elif (( wlen >= 3 && wlen <= 4 )) || (( wlen >= 8 && wlen <= 9 )); then
                    chance=30
                else
                    chance=10
                fi
                if (( RANDOM % 100 < chance )); then
                    if (( 12 - ${#line} >= wlen )); then
                        line+="$w"
                        ((word_count++))
                        continue
                    else
                        # Le mot ne rentre pas, on le "porte" à la colonne suivante
                        carry_word="$w"
                        carry_wlen=$wlen
                        break
                    fi
            ...
            # Si pas d'insertion de mot, caractère buggué
            prev_char=""
            if [[ ${#line} -gt 0 ]]; then
                prev_char="${line: -1}"
            fi
            for try in {1..10}; do
                c=${BUGGED_CHARS[$((RANDOM % ${#BUGGED_CHARS[@]}))]}
                # 1% chance to allow consecutive identiques
                if [[ "$c" != "$prev_char" || $((RANDOM % 100)) -eq 0 ]]; then
                    break
                fi
            done
            line+="$c"
        done
        # Si la ligne est plus courte que 12 (à cause d'un break pour porter un mot), on complète avec des caractères buggués
        while [[ ${#line} -lt 12 ]]; do
            prev_char=""
            if [[ ${#line} -gt 0 ]]; then
                prev_char="${line: -1}"
...
# Fallout-style header above the grid
block='\u2588'
ATTEMPTS=4

print_header_and_attempts() {
    echo -e "[MOUNT DESERT ISLAND NAVAL FACILITY]"
    echo
    echo -e "Password Required"
    echo
    attempts_bar=""
    for ((a=0; a<ATTEMPTS; a++)); do
        attempts_bar+=" \e[33m$block\e[0m"
    done
    echo -e "Attemps remaining : $attempts_bar"
    echo
}

print_header_and_attempts

# Print all columns side by side, two spaces between each (11 lines)
for ((i=0; i<11; i++)); do
    printf "%-6s  %-12s  %-6s  %s\n" "${LEFT_COL[$i]}" "${RIGHT_COL[$i]}" "${MID_COL[$i]}" "${FAR_COL[$i]}"
done


# Pick a password from the words used in the grid (only full words, not substrings)
GRID_WORDS=()
extract_words_from_line() {
    # Extrait toutes les séquences de lettres majuscules (mots) d'une ligne
    echo "$1" | grep -oE '[A-Z]+'
}
for line in "${RIGHT_COL[@]}" "${FAR_COL[@]}"; do
    for found in $(extract_words_from_line "$line"); do
        for w in "${WORDS[@]}"; do
            if [[ "$found" == "$w" ]]; then
                GRID_WORDS+=("$w")
            fi
...
GRID_WORDS=($(printf "%s\n" "${GRID_WORDS[@]}" | awk '!seen[$0]++'))
PASSWORD=$(shuf -e "${GRID_WORDS[@]}" | head -n1)


clear
echo -e "\e[36m[DEBUG] PASSWORD: $PASSWORD\e[0m"
declare -a HISTORY_GUESS=()
declare -a HISTORY_LIKENESS=()
while (( ATTEMPTS > 0 )); do
    print_header_and_attempts
    # Afficher la grille
    for ((i=0; i<11; i++)); do
    ...
    echo
    # Afficher l'historique juste après la grille
    if (( ${#HISTORY_GUESS[@]} > 0 )); then
        echo "--- Previous attempts ---"
        for ((h=0; h<${#HISTORY_GUESS[@]}; h++)); do
            echo -e "${HISTORY_GUESS[$h]}  -  Likeness: ${HISTORY_LIKENESS[$h]}"
        done
        echo
    fi
    ...
    get_password_guess() {
        local GUESS=""
        local prompt=$'\e[32m> ENTER PASSWORD (Tab to quit): \e[0m'
        echo -ne "$prompt"
        while true; do
            IFS= read -rsn1 key
            # Tab = quitter
            if [[ "$key" == $'\t' ]]; then
                echo -e "\n\e[33m[QUIT] Game exited by user (Tab)\e[0m"
                read -rp $'\nPress Enter to exit...'
                exit 0
            fi
            # Entrée = valider
            ...
            # Si c'est un caractère imprimable (ASCII visible)
            if [[ "$key" =~ ^[[:print:]]$ ]]; then
                GUESS+="$key"
                echo -n "$key"
            fi
    ...
    GUESS=$(get_password_guess)
    GUESS_UPPER=$(echo "$GUESS" | tr '[:lower:]' '[:upper:]')
    PASS_UPPER=$(echo "$PASSWORD" | tr '[:lower:]' '[:upper:]')
    if [[ "$GUESS_UPPER" == "$PASS_UPPER" ]]; then
        echo -e "\e[32mACCESS GRANTED\e[0m"
        aplay -q /home/noa/Fallout3Terminal/ui_hacking_passgood.wav 2>/dev/null || play -q /home/noa/Fallout3Terminal/ui_hacking_passgood.wav 2>/dev/null || true
        read -rp $'\nPress Enter to exit...'
        exit 0
    else
        # Likeness: count matching chars in same position
        likeness=0
        minlen=${#GUESS_UPPER}
        [[ ${#PASS_UPPER} -lt $minlen ]] && minlen=${#PASS_UPPER}
        for ((i=0; i<minlen; i++)); do
            if [[ "${GUESS_UPPER:$i:1}" == "${PASS_UPPER:$i:1}" ]]; then
                ((likeness++))
            fi
        done
        HISTORY_GUESS+=("$GUESS_UPPER")
        HISTORY_LIKENESS+=($likeness)
        ((ATTEMPTS--))
        echo -e "\e[31mACCESS DENIED\e[0m - Likeness: $likeness"
        aplay -q /home/noa/Fallout3Terminal/ui_hacking_passbad.wav 2>/dev/null || play -q /home/noa/Fallout3Terminal/ui_hacking_passbad.wav 2>/dev/null || true
        sleep 1
        clear
    fi
done
echo -e "\e[31mTERMINAL LOCKED\e[0m"
# Wait for user input before exiting, so the terminal doesn't close immediately
